...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    vec![PluginSource {
        name: "cevizdesk".to_string(),
        url: "https://raw.githubusercontent.com/fufesou/cevizdesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "Cevizdesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "Cevizdesk wants to uninstall the plugin",
    )
}
